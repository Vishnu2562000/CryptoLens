...
const projectSecretID = process.env.NEXT_PUBLIC_IPFS_API_KEY;
const auth = `Basic ${Buffer.from(`${projectID}:${projectSecretID}`).toString('base64')}`;
...
const CreateItem = () => {
  ...
  const uploadToInfura = async (file) => {
    const subDomain = 'https://crypto-lens-nft-marketplace.infura-ipfs.io';
    try {
